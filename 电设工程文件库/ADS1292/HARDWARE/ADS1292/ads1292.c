...
void ADS1292_Init(void)
{
	...
	// ADS1292_DRDY -> PE6
	GPIO_InitStruct.Pin   = GPIO_PIN_6;       // 配置ADS1292_DRDY
...
void ADS1292_PowerOnInit(void)
{
	...
	ADS1292_START(0);
	ADS1292_CS(0);
	SPI4_Send_Byte(SDATAC); // 发送停止连续读取数据命令
	delay_us(10);
	ADS1292_CS(1);
...
{
	ADS1292_CS(0);				// 片选拉低
	SPI4_Send_Byte(addr);	// 包含命令操作码和寄存器地址
	delay_us(10);
	SPI4_Send_Byte(0x00);	// 要读取的寄存器数+1
	delay_us(10);
	SPI4_Send_Byte(data);	// 写入的数据
	delay_us(10);
	ADS1292_CS(1);				// 片选置高
}
...
void ADS1292_Read_Data(u8 *data)
{
	u8 i;
	ADS1292_CS(0);
	SPI4_Send_Byte(RDATAC);		// 发送启动连续读取数据命令
	delay_us(10);
	ADS1292_CS(1);						
	ADS1292_START(1); 				// 启动转换
	while (DYDR_READ == 1);	// 等待DRDY信号拉低
	ADS1292_CS(0);
