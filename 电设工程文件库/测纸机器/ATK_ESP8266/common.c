...
void atk_8266_msg_show(u16 x,u16 y,u8 wanip)
{
	...
	delay_ms(800);//延时1.5s等待模块重启
	u2_print("page0.t1.txt=\"等待模块重启\"");
	delay_ms(400);//
	delay_ms(1000);
...
u8 atk_8266_netpro_sel(u16 x,u16 y,u8* name) 
{
	...
	p1=mymalloc(SRAMIN,50);//申请50个字节的内存
...
void accept(u8* p)
{
	...
	{ 
		...
		printf("%s",USART3_RX_BUF);	//发送到串口   
		sprintf((char*)p,"收到%d字节,内容如下",rlen);//接收到的字节数,同时把内容考到P里面 
		USART3_RX_STA=0;
	} 
...
void atk_8266_test(void)
{
	u8 *p,*p1;
	u8 flag;
	p=mymalloc(SRAMIN,50);		//申请动态内存空间50字节
	p1=mymalloc(SRAMIN,50);		//申请动态内存空间50字节
	printf("检查WIFI模块是否在线");
	...
	delay_ms(2000);//等待两秒
	printf("检查模式\r\n");
	if(atk_8266_send_cmd("AT+CWMODE=2","2",20)==1)//模式不对，打印模式结果
	{
	...
	sprintf((char*)p,"ATK-8266%s测试\r\n",ATK_ESP8266_WORKMODE_TBL[0]);//测试数据
	//Show_Str(30+54,100,200,12,p,12,0);
	atk_8266_send_cmd("AT+CIPSEND=0,25","OK",200);  //发送指定长度的数据
	delay_ms(200);
	flag=atk_8266_send_data(p,"OK",100);  //发送指定长度的数据
	if(flag==0)
		printf("测试成功\r\n");
